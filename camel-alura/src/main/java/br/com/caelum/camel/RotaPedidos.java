...
public class RotaPedidos {
	...
	public static void main(String[] args) throws Exception {
		...
		//Para acessar o admin do ActiveMQ: http://127.0.0.1:8161/admin;
		context.addComponent("activemq", ActiveMQComponent.activeMQComponent("tcp://localhost:9191"));
		context.addRoutes(new RouteBuilder() {
			@Override
			public void configure() throws Exception {

				this.errorHandler(deadLetterChannel("activemq:queue:pedidos.DLQ")
				...
				this.from("file:pedidos?delay=5s&noop=true")
						.routeId("rota-arquivo-pedidos")
						.delay(2000L)
						.to("activemq:queue:pedidos");

				this.from("activemq:queue:pedidos")
